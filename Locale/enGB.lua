...
L["TricksGuideSection_6"] = "The fiest option requires all materials to be have their custom price set. Pros: You need to setup it once. Cons - items you need to define price sources for those items that'd be always used by this addon. You need to have all materials you can loot before the farm (or you can have a break to configure custom prices as soon as you loot those items). The trick is that items with custom price source configurated are recorded regardless of your min quality/min price settings for Recorder. Go to Configuration -> Custom Prices. Drop items that you want to record onto the empty slot. They'll be added items to the list. You can select Edit from menu next to that item to change a price source for that item. Then go to Configuration -> Recorder and set Min Quality to Rare or Epic (common/uncommon materials with a custom price will be recorder anyway)."
